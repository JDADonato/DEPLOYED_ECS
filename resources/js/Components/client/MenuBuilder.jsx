...
const MenuBuilder = ({ bookingData, updateBooking, onNext, onBack, mode = 'full' }) => {
    ...
    useEffect(() => {
        ...
        fetch('/api/settings/business-rules')
            ...
            .catch(error => console.error('Error fetching business rules:', error));
    ...
    const selectedDishCount = Object.values(selections).reduce((sum, arr) => sum + (arr?.length || 0), 0);
    ...
    return (
        <div className="flex flex-col h-full animate-fadeIn">
            ...
            {/* Over Budget Warning */}
            {budget && parseInt(budget) > 0 && (Math.ceil(menuTotal * (1 + (packageContextFields.package_vat_rate || 0) + (packageContextFields.package_service_charge_rate || 0) + (packageContextFields.package_security_rate || 0))) + (packageContextFields.package_cash_bond || 0) > parseInt(budget)) && (
