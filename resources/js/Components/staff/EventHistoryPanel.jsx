    ...
    return (
        <div className={isAdminSurface ? 'admin-embedded-surface' : 'staff-work-surface'}>
            ...
            <div className={isAdminSurface ? 'admin-command-strip event-history-filter-bar' : 'staff-filter-bar event-history-filter-bar'}>
                ...
                <div className="flex items-center gap-2" style={{ flex: '2 0 18rem' }}>
                    <input type="date" value={filters.date_from} onChange={(event) => setFilter('date_from', event.target.value)} className="staff-control flex-1 min-w-0" />
                    <span aria-hidden="true" className="text-[0.68rem] font-black text-slate-400 uppercase">to</span>
                    <input type="date" value={filters.date_to} onChange={(event) => setFilter('date_to', event.target.value)} className="staff-control flex-1 min-w-0" />
                </div>
                {role !== 'accounting' && (
                    <>
