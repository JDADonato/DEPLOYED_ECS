    ...
    <RevealOnScroll as="section" delay={revealDelay} className={`admin-panel admin-analytics-panel ${className}`}>
        <div className="admin-analytics-panel-head">
            ...
            <div className="admin-analytics-panel-actions">
                {actions}
                {onExpand && (
                    <button type="button" onClick={() => onExpand(id)} className="admin-mini-button inline-flex items-center gap-2">
    ...
    const renderAnalyticsFilterPanel = (panel) => {
                ...
                <div className="space-y-3">
                    ...
                    {analyticsFilters.snapshot_window === 'custom' && (
                        <div className="grid grid-cols-2 gap-2">
                            <label className={analyticsFilterLabelClass}>
                                From
                                <input type="date" value={analyticsFilters.date_from || ''} onChange={(event) => updateAnalyticsFilters({ date_from: event.target.value })} className={analyticsFilterInputClass} />
                            </label>
                            <label className={analyticsFilterLabelClass}>
                                To
                                <input type="date" value={analyticsFilters.date_to || ''} onChange={(event) => updateAnalyticsFilters({ date_to: event.target.value })} className={analyticsFilterInputClass} />
                            </label>
                        </div>
                    )}
                </div>,
        ...
        if (panel === 'dashboardPayment' || panel === 'paymentRisk') {
            return tray(
                <>
                    <label className={analyticsFilterLabelClass}>
                        Payment status
                        <select value={analyticsFilters.breakdown_payment_status || ''} onChange={(event) => updateAnalyticsFilters({ breakdown_payment_status: event.target.value }, 'dashboardPayment')} className={analyticsFilterInputClass}>
        ...
        if (panel === 'bookingPipeline') {
            return tray(
                <label className={analyticsFilterLabelClass}>
                    Booking status
                    <select value={analyticsFilters.pipeline_booking_status || ''} onChange={(event) => updateAnalyticsFilters({ pipeline_booking_status: event.target.value }, panel)} className={analyticsFilterInputClass}>
            ...
            return tray(
                <label className={analyticsFilterLabelClass}>
                    ...
                    <select
                        ...
                        onChange={(event) => updateAnalyticsFilters({ sales_package_category: event.target.value })}
                        className={analyticsFilterInputClass}
                    >
        ...
        if (panel === 'conversionFunnel') {
            return tray(
                <label className={analyticsFilterLabelClass}>
                    Funnel timeframe
                    <select value={analyticsFilters.funnel_snapshot_window} onChange={(event) => updateAnalyticsFilters({ funnel_snapshot_window: event.target.value }, panel)} className={analyticsFilterInputClass}>
        ...
        if (panel === 'packagePerformance') {
            return tray(
                <>
                    <label className={analyticsFilterLabelClass}>
                        Package category
                        <select value={analyticsFilters.performance_package_category || ''} onChange={(event) => updateAnalyticsFilters({ performance_package_category: event.target.value })} className={analyticsFilterInputClass}>
        ...
        if (panel === 'revenueRegression') {
            return tray(
                <label className={analyticsFilterLabelClass}>
                    Regression history
                    <select value={analyticsFilters.revenue_forecast_months} onChange={(event) => {
                        const months = parseInt(event.target.value, 10);
                        const period = months > 12 ? 'quarterly' : 'monthly';
                        updateAnalyticsFilters({ 
                            revenue_forecast_months: event.target.value,
                            revenue_forecast_period: period,
                        }, panel);
                    }} className={analyticsFilterInputClass}>
                        {[6, 9, 12, 18, 24].map(months => <option key={months} value={months}>{months} months history + {months > 12 ? '3 quarters' : '3 months'} projected</option>)}
                    </select>
                </label>,
                'sm:grid-cols-1'
            );
        ...
        if (panel === 'paxForecast') {
            return tray(
                <>
                    <label className={analyticsFilterLabelClass}>
                        Forecast period
                        <select value={analyticsFilters.pax_projection_period} onChange={(event) => updateAnalyticsFilters({ pax_projection_period: event.target.value }, panel)} className={analyticsFilterInputClass}>
                    ...
                    <label className={analyticsFilterLabelClass}>
                        Year
                        <select value={analyticsFilters.pax_projection_year} onChange={(event) => updateAnalyticsFilters({ pax_projection_year: event.target.value }, panel)} className={analyticsFilterInputClass}>
                    ...
                    <label className={analyticsFilterLabelClass}>
                        Quarter
                        <select value={analyticsFilters.pax_projection_quarter} onChange={(event) => updateAnalyticsFilters({ pax_projection_quarter: event.target.value }, panel)} className={analyticsFilterInputClass}>
                    ...
                    <label className={analyticsFilterLabelClass}>
                        SMA window
                        <select value={analyticsFilters.pax_sma_window} onChange={(event) => updateAnalyticsFilters({ pax_sma_window: event.target.value }, panel)} className={analyticsFilterInputClass}>
                            {[2, 3, 4, 6].map(window => <option key={window} value={window}>{window} periods</option>)}
                        </select>
                    </label>
                    <label className={analyticsFilterLabelClass}>
                        Projection horizon
                        <select value={analyticsFilters.pax_projection_horizon} onChange={(event) => updateAnalyticsFilters({ pax_projection_horizon: event.target.value }, panel)} className={analyticsFilterInputClass}>
    ...
    const renderAnalyticsWorkbench = () => {
        const insightCards = [
            ...
            },
            {
                ...
                tooltip: 'Number of active confirmed bookings for events within the selected timeframe.',
            },
            {
                key: 'collection',
                title: 'Collection health',
                value: `${Number(analyticsSummary.collectionRate || 0).toFixed(1)}%`,
                context: 'Collection rate based on verified and pending payment records.',
                action: 'View finance',
                onClick: () => setActiveTab('finance'),
                tooltip: 'Percentage of expected revenue that has been successfully collected for the selected timeframe.',
            },
            {
                key: 'demand',
                title: 'Guest demand',
                value: (analyticsSummary.totalPax || 0).toLocaleString(),
                ...
                tooltip: 'Total number of guests (pax) expected across all bookings in the selected timeframe.',
            },
        ];
                        ...
                        <>
                            ...
                            <div className="admin-analytics-metric-strip">
                                {insightCards.map((card, index) => (
                                    <RevealOnScroll as="article" key={card.key} delay={`rv-d${Math.min(index + 1, 4)}`} className="admin-analytics-metric-card" title={card.tooltip}>
                                        <p title={card.tooltip}>{card.title} <span className="ml-1 inline-flex cursor-help items-center justify-center rounded-full bg-gray-100 text-gray-500 h-4 w-4 text-[10px]">?</span></p>
                                        <strong>{card.value}</strong>
                                        <p className="admin-metric-context">{card.context}</p>
