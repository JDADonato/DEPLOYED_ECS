...
const DashboardMarketing = () => {
    ...
    const closeConfirmDialog = () => setConfirmDialog({ isOpen: false, title: '', message: '', confirmText: 'Confirm', tone: 'default', busy: false, onConfirm: null });
    ...
    const [rejectConfirmBookingId, setRejectConfirmBookingId] = useState(null);
    ...
    // Removed updateLead logic as it is now in GuestInquiriesManager
    const handleLogout = () => {
        logoutWithCleanup();
    };
    ...
    const handleArchiveEventType = async (eventType) => {
        setConfirmDialog({
            ...
            message: 'This hides the event type from future customer booking choices while preserving historical bookings.',
            confirmText: 'Archive',
            tone: 'danger',
    ...
    const confirmArchiveEventType = async (eventType) => {
        ...
        setSettingsSaving(true);
        try {
            const response = await csrfFetch(`/api/settings/event-types/${eventType.id}/archive`, { method: 'PATCH' });
            const data = await response.json().catch(() => ({}));
    ...
    const confirmUnarchiveEventType = async (eventType) => {
        ...
        setSettingsSaving(true);
        try {
            const response = await csrfFetch(`/api/settings/event-types/${eventType.id}/unarchive`, { method: 'PATCH' });
            const data = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(data.error || data.message || 'Could not restore event type.');
            toast.success(data.message || 'Event type restored.');
            bustMarketingSettingsCache();
            fetchMarketingSettings();
        } catch (error) {
            console.error('Error restoring event type:', error);
            toast.error(error.message || 'Could not restore event type.');
        } finally {
            setSettingsSaving(false);
        }
    };
    ...
    const confirmDeleteEventType = async (eventType) => {
        ...
        setSettingsSaving(true);
        try {
            const response = await csrfFetch(`/api/settings/event-types/${eventType.id}/destroy`, { method: 'DELETE' });
            const data = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(data.error || data.message || 'Could not delete event type.');
            toast.success(data.message || 'Event type permanently deleted.');
            bustMarketingSettingsCache();
            fetchMarketingSettings();
        } catch (error) {
                    ...
                    <>
                        ...
                        {canEdit && !['Cancelled', 'Completed'].includes(selectedBooking.status) && String(selectedBooking.review_status || '').toLowerCase() !== 'approved for reservation' && (
                            <>
                                ...
                                <button onClick={() => setRejectConfirmBookingId(selectedBooking.id)} className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-black text-rose-700 hover:bg-rose-100">
                                    Reject
                                </button>
                            </>
                        )}
    ...
    // renderPublicLeads extracted to GuestInquiriesManager
        ...
        return (
                                                ...
                                                <div className="relative inline-block text-left">
                                                    ...
                                                    {openDropdownId === type.id && (
                                                        <div 
                                                            ...
                                                            <button onClick={() => { handleDeleteEventType(type); setOpenDropdownId(null); }} className="border-t border-gray-100 px-4 py-2.5 text-left text-xs font-bold text-red-600 transition-colors hover:bg-red-50">Delete</button>
                                                        </div>
                                                    )}
                                                </div>
                                            </td>
            ...
            {false && <>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-3 gap-3">
                        {eventTypes.map(type => (
                            <div key={type.id} className={`rounded-lg border p-4 ${type.is_active === false ? 'border-red-100 bg-red-50/30' : 'border-gray-100 bg-gray-50'}`}>
                                <div className="flex items-center gap-2">
                                    <p className="text-xs font-black uppercase text-primary-600">{type.slug}</p>
                                    {type.is_active === false && <span className="rounded bg-red-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-red-700">Archived</span>}
                                </div>
                                ...
                                <div className="mt-3 flex flex-wrap gap-2">
                                    ...
                                    {type.is_active === false ? (
                                        <button onClick={() => handleUnarchiveEventType(type)} className="rounded-lg bg-green-50 px-3 py-2 text-xs font-bold text-green-700 hover:bg-green-100">Restore</button>
                                    ) : (
                                        <button onClick={() => handleArchiveEventType(type)} className="rounded-lg bg-orange-50 px-3 py-2 text-xs font-bold text-orange-700 hover:bg-orange-100">Archive</button>
                                    )}
        ...
        >
                <StaffPageHeader
                    ...
                    metrics={[
                        ...
                        { label: activeTab === 'calendar' ? (() => {
                            const [y, m] = selectedMonthKey.split('-');
                            const d = new Date(y, parseInt(m, 10) - 1, 1);
                            return d.toLocaleDateString('en-US', { month: 'short' }).toUpperCase() + ' EVENTS';
                        })() : 'This Month', value: dashboardSummary.monthEvents, helpText: 'Events scheduled in the currently selected calendar month.' },
                ...
                {activeTab === 'leads' && <GuestInquiriesManager />}
                {activeTab === 'tastings' && (
            ...
            <ConfirmModal
                ...
                busy={settingsSaving || confirmDialog.busy}
                onCancel={closeConfirmDialog}
                onConfirm={confirmDialog.onConfirm}
            />
            <FlashToast />
            <DangerConfirmModal
                isOpen={!!rejectConfirmBookingId}
                title="Reject Booking Request?"
                message="This will reject the booking request and notify the customer. This action can be undone later from the event history."
                onConfirm={() => {
                    if (rejectConfirmBookingId) {
                        updateStatus(rejectConfirmBookingId, 'Cancelled');
                        setRejectConfirmBookingId(null);
                    }
                }}
                onCancel={() => setRejectConfirmBookingId(null)}
            />
        </StaffWorkspaceLayout>
    );
};
