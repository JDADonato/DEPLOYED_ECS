...
const useStaffContextNavigation = ({
    ...
    const applyContextParams = useCallback((rawParams = {}, options = {}) => {
        ...
        onApplyContext?.({ params, targetTab, context, searchText, ...options });
    ...
    useEffect(() => {
        ...
        const handleNavigationQueryChange = (event) => {
            ...
            applyContextParams(event.detail?.params || new URLSearchParams(event.detail?.search || window.location.search), { fromNavigationEvent: true });
        };
