...
class ChatController extends Controller
{
    ...
    public function conversations(Request $request)
    {
        ...
        $payload = [
            ...
            'resolved' => $this->getMyResolvedChats($user, $limit),
        ];
    ...
    private function getMessagePreviewText(?Message $message): string
    {
        if (! $message) {
            return '';
        }
        ...
        if (Str::startsWith($text, '{') && Str::contains($text, '"type"')) {
            $data = json_decode($text, true);
            if (isset($data['type'])) {
                if ($data['type'] === 'booking_details') {
                    return 'Sent booking details';
                }
                if ($data['type'] === 'system') {
                    return $data['text'] ?? 'System message';
                }
            }
        }
    ...
    private function formatConversation(Conversation $conv, $currentUser): array
    {
        ...
        return [
            ...
            'last_message' => $this->getMessagePreviewText($conv->latestMessage),
    ...
    private function getMyResolvedChats($user, int $limit = 25): array
    {
        return Conversation::resolved()
            ->where(function ($inner) use ($user) {
                $inner->where('staff_id', $user->id)
                    ->orWhereHas('participantHistory', function ($participant) use ($user) {
                        $participant->where('user_id', $user->id)
                            ->whereIn('role', ['owner', 'collaborator']);
                    });
            })
            ->with(['client:id,full_name,username,email,phone,account_status,deactivated_at', 'staff:id,username,full_name,role,account_status', 'booking:id,user_id,event_name,event_type,event_date,status,client_full_name,client_email,client_phone', 'latestMessage.sender:id,username', 'participants.user:id,username,full_name,role,account_status', 'participantHistory.user:id,username,full_name,role,account_status'])
            ->withCount(['messages as unread_count' => function ($q) use ($user) {
                $q->where('sender_id', '!=', $user->id)->whereNull('read_at');
            }])
            ->latest('updated_at')
