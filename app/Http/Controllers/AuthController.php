...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $emailDelivery = app(EmailDeliveryService::class)
            ->sendMailable($user->email, new VerifyEmailOTP($otp, 'account verification', 15), 'otp_registration');

        Log::info('OTP registration email delivery.', ['user_id' => $user->id, 'status' => $emailDelivery['status']]);
        ...
        $errorMessage = $emailDelivery['message'] ?? 'We could not send the first verification code. Please use Resend Code to try again.';
        if (str_contains($errorMessage, 'Copy and share') || str_contains($errorMessage, 'Copy the password')) {
            $errorMessage = 'Email could not be delivered due to a server configuration issue. Please contact support or try again later.';
        }
    ...
    public function resendOtp(Request $request)
    {
        ...
        $emailDelivery = app(EmailDeliveryService::class)
            ->sendMailable($user->email, new VerifyEmailOTP($otp, 'account verification', 15), 'otp_resend');

        Log::info('OTP resend email delivery.', ['user_id' => $user->id, 'status' => $emailDelivery['status']]);

        if ($emailDelivery['status'] !== 'sent') {
            $errorMessage = $emailDelivery['message'] ?? 'We could not send a verification code right now. Please try again.';
            if (str_contains($errorMessage, 'Copy and share') || str_contains($errorMessage, 'Copy the password')) {
                $errorMessage = 'Email could not be delivered due to a server configuration issue. Please contact support or try again later.';
            }
