...
class AccountingController extends Controller
{
    ...
    {
        // Build column list dynamically so the query works even if migrations haven't run yet
        $bookingColumns = [
            'id',
            'user_id',
            'event_date',
        ...
        if (\Schema::hasColumn('bookings', 'requires_accounting_review')) {
            $bookingColumns[] = 'requires_accounting_review';
            $bookingColumns[] = 'last_modified_reason';
        }
        ...
        if (\Schema::hasColumn('payments', 'receipt_type')) {
            $paymentColumns[] = 'receipt_type';
            $paymentColumns[] = 'receipt_url';
        }

        $query = Booking::query()
            ->select($bookingColumns)
            ->with(['user:id,full_name,username,email,phone,account_status', 'payments' => function ($q) use ($request, $paymentColumns) {
    ...
    /**
     * Acknowledge change order and clear the requires_accounting_review flag.
     */
    public function acknowledgeChangeOrder(int $id)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Change order acknowledged.']);
    }
