...
class SettingsController extends Controller
{
    ...
    public function updateSurchargeRules(Request $request)
    {
        $data = $request->validate([
            'location_surcharge_rate' => 'required|numeric|min:0|max:1',
            'floor_surcharge_rate' => 'required|numeric|min:0|max:1',
        ...
        app(OperationalBroadcastService::class)
            ->adminChanged('finance', 'surcharge_rules', $rule->id, 'updated', 'Surcharge configurations updated.');
    ...
    public function unarchiveEventType(int $id)
    {
        $eventType = EventType::findOrFail($id);
        $eventType->forceFill([
            'is_active' => true,
        ...
        $this->bumpCatalogVersion();
        app(\App\Services\OperationalBroadcastService::class)->adminChanged('catalog', 'event_type', $eventType->id, 'unarchived', 'Event type restored.');

        return response()->json(['message' => 'Event type restored successfully.', 'event_type' => $eventType->fresh()]);
    }

    public function destroyEventType(int $id)
    {
        $eventType = EventType::findOrFail($id);
        $eventType->delete();
        $this->bumpCatalogVersion();
        app(\App\Services\OperationalBroadcastService::class)->adminChanged('catalog', 'event_type', $id, 'deleted', 'Event type permanently deleted.');

        return response()->json(['message' => 'Event type permanently deleted.']);
    }
