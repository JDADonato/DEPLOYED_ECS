...
- Set `PERSISTENT_STORAGE_PATH` to an absolute or relative path outside your Git deployment directory (e.g. `../../ecs_persistent_storage`) if using Hostinger's auto-deploy, so user-uploaded files are not wiped on every deploy.
